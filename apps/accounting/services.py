...
def post_journal(*, narration: str, lines: list[dict], source_type: str = "", source_id: int | None = None, entry_date=None, created_by=None) -> JournalEntry:
    ...
    journal = JournalEntry.objects.create(
        entry_number=_next_journal_number(), entry_date=entry_date or timezone.localdate(), narration=narration,
    ...
    for line in lines:
        ...
        LedgerEntry.objects.create(
            journal_entry=journal, account=account, customer=line.get("customer"), supplier=line.get("supplier"),
            debit=debit, credit=credit, transactional_balance=balances[account.pk], line_memo=line.get("memo", ""),
        )
    return journal
...
def post_customer_receipt(receipt: CustomerReceipt, created_by=None) -> CustomerReceipt:
    ...
    journal = post_journal(
        narration=f"Customer receipt from {receipt.customer.name}", source_type="customer_receipt", source_id=receipt.pk,
        entry_date=receipt.receipt_date, created_by=created_by,
        lines=[
            {"account": accounts["CASH"], "debit": receipt.amount, "credit": 0, "customer": receipt.customer},
            {"account": accounts["AR"], "debit": 0, "credit": receipt.amount, "customer": receipt.customer},
        ],
    )
    receipt.journal_entry = journal
    receipt.save(update_fields=["journal_entry", "updated_at"])
    remaining = receipt.amount
    old_records = OldUdhaar.objects.select_for_update().filter(customer=receipt.customer).order_by("opening_date", "id")
...
def record_old_udhaar(customer, amount: Decimal, note: str = "Opening balance", created_by=None, opening_date=None) -> OldUdhaar:
    ...
    accounts = ensure_system_accounts()
    journal = post_journal(
        narration=f"Opening udhaar for {customer.name}", source_type="old_udhaar", source_id=old.pk, entry_date=old.opening_date, created_by=created_by,
        lines=[
            {"account": accounts["AR"], "debit": amount, "credit": 0, "customer": customer},
            {"account": accounts["OPENING_EQUITY"], "debit": 0, "credit": amount, "customer": customer},
        ],
    )
